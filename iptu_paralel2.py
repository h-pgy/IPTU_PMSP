...
def proc_func(batches_proc, df_log, ano, file_iptu):
    
    for batch in batches_proc:
        gerar_csv_bairros(batch,
                         df_log, 
                         ano, 
                         file_iptu
                         )

def gerar_procs(batch_bairros, num_procs, df_log, ano, file_iptu):
    
    procs = []
    start = 0
    if num_procs > 1:
        for i in range(len(batch_bairros)+1):
            if (i>0) and (i%(len(batch_bairros)//num_procs)==0):
                stop = i
                batches_proc = batch_bairros[start:stop]
                p = Process(target = proc_func, args = (batches_proc, df_log, ano, file_iptu))
                procs.append(p)
                start = i
    return procs

    
def main(file_iptu, num_batches, num_procs):
    
    df_log, bairros_batch, ano = inicializacao(file_iptu, num_batches)
    procs = gerar_procs(bairros_batch, num_procs, 
                        df_log, ano, file_iptu)
    
    for i, proc in enumerate(procs):
        
        print('Inicializando processo:', i)
        proc.start()

if __name__ == "__main__":
    
    file = 'IPTUS/EG2018.csv'
    main(file, num_batches = 30, num_procs = 3)
